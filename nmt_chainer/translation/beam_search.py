...
def update_next_lists(num_case, idx_in_case, new_cost, eos_idx, new_state_ensemble, finished_translations, current_translations,
                      ...
                      attn_ensemble, next_attentions_list, graph_data, beam_score_coverage_penalty, beam_score_coverage_penalty_strength, need_attention=False):
    """
    ...
    Args:
        ...
        graph_data structure to accumulate data to generate a resolution graph
...
def compute_next_lists(new_state_ensemble, new_scores, beam_width, beam_pruning_margin,
    ...
    """
        ...
        Args:
            ...
            graph_data: whole resolution graph
    ...
    for num_case, idx_in_case, new_cost in score_iterator:
        ...
        update_next_lists(num_case, idx_in_case, new_cost, eos_idx, new_state_ensemble,
                          ...
                          next_states_list, next_words_list, next_score_list, next_normalized_score_list, next_translations_list,
                          attn_ensemble, next_attentions_list, graph_data, beam_score_coverage_penalty, beam_score_coverage_penalty_strength, need_attention=need_attention)
...
#     xp = cuda.get_array_module(dec_ensemble[0].initial_state.data)
    xp = dec_cell_ensemble[0].xp
    current_translations, current_scores, current_states_ensemble, current_words, current_attentions, graph_data = current_translations_states
    ...
    next_states_list, next_words_list, next_score_list, next_translations_list, next_attentions_list = compute_next_lists(
        ...
        current_attentions, graph_data, attn_ensemble, force_finish=force_finish, need_attention=need_attention,
        num_step=num_step)
