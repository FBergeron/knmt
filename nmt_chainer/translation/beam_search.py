...
def update_next_lists(num_case, idx_in_case, new_cost, eos_idx, new_state_ensemble, finished_translations, current_translations,
                      current_attentions, current_score_lists,
                      next_states_list, next_words_list, next_score_list, next_normalized_score_list, next_translations_list,
                      attn_ensemble, next_attentions_list, tree_data, next_score_lists_list, beam_score_coverage_penalty, beam_score_coverage_penalty_strength, need_attention=False):
    """
    ...
    Args:
        ...
        tree_data structure to accumulate data to generate a resolution graph
        next_score_lists_list: list of list of scores.
...
def compute_next_lists(new_state_ensemble, new_scores, beam_width, beam_pruning_margin,
    ...
    """
        ...
        Args:
            ...
            tree_data: whole resolution graph
            current_score_lists: list of scores for the partial translations in the current beam.
                                    each item is a sequence of scores.
    ...
    for num_case, idx_in_case, new_cost in score_iterator:
        ...
        update_next_lists(num_case, idx_in_case, new_cost, eos_idx, new_state_ensemble,
                          finished_translations, current_translations, current_attentions, current_score_lists,
                          next_states_list, next_words_list, next_score_list, next_normalized_score_list, next_translations_list,
                          attn_ensemble, next_attentions_list, tree_data, next_score_lists_list, beam_score_coverage_penalty, beam_score_coverage_penalty_strength, need_attention=need_attention)
...
#                 break
    ...
    return next_states_list, next_words_list, next_score_list, next_translations_list, next_attentions_list, next_score_lists_list
...
#     xp = cuda.get_array_module(dec_ensemble[0].initial_state.data)
    ...
    next_states_list, next_words_list, next_score_list, next_translations_list, next_attentions_list, next_score_lists_list = compute_next_lists(
        new_state_ensemble, new_scores, beam_width, beam_pruning_margin,
        beam_score_length_normalization, beam_score_length_normalization_strength,
        beam_score_coverage_penalty, beam_score_coverage_penalty_strength,
        eos_idx,
        current_translations, finished_translations,
        current_attentions, tree_data, current_score_lists, attn_ensemble, force_finish=force_finish, need_attention=need_attention,
        num_step=num_step)
...
def ensemble_beam_search(model_ensemble, src_batch, src_mask, nb_steps, eos_idx,
    ...
    with chainer.using_config("train", False), chainer.no_backprop_mode():
        ...
        current_translations_states = (
            ...
            None,  # previous words
            [[]],  # attention
            tree_data, # data used to build resolution tree
            [[]] # score_lists
        )
